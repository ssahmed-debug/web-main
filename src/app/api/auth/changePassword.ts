...
const POST = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    ...
    if (passwordToCompare) {
      // if the compare have value, it means we only want this api to compare the password and not change it.
      ...
      await UserSchema.findOneAndUpdate({ _id }, { password: hashedPassword });
      return res
        .status(200)
